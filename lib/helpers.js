...
//DOESN'T WORK BECAUSE CRYPTO IS PRIVATE
helpers.setTopic = (bot, topic) => {
    var top = topic;
    var chan = process.env.CRYPTO_CHAN;
    console.log(`specifying topic ${top}`);
    console.log(`in channel ${chan}`);
    bot.api.channels.setTopic({
        channel: chan,
        topic: top
    }, (err, res) => {
        if (err) bot.botkit.log("Failed to set topic: (", err)
    });
}
...
helpers.giphy = (query, bot, message) => {
    request("http://api.giphy.com/v1/gifs/search?q=" + query + "&api_key=dc6zaTOxFJmzC", function (error, response, body) {
        ...
        if (max > 0) {
            ...
            bot.reply(`Sorry, couldn't find any gifs matching ${query} `)
        }
    });
}
