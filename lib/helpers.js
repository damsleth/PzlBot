...
helpers.getUserInfo = function (bot, message) {
    var userIDToFind = message.match[1];
    request.get(`https://slack.com/api/users.info?user=${userIDToFind}&token=${process.env.SLACK_TOKEN}`, function (error, response, body) {
        if (!error) {
            console.log(`retrieved UserInfo for ${userIDToFind} on behalf of ${message.user}`)
            var formattedBody = helpers.replaceAllInStr(body, ",", ",\n");
            formattedBody = helpers.replaceAllInStr(formattedBody, "{", "{\n");
            bot.reply(message, `Here's what i have on user with ID ${userIDToFind}: \n  ${formattedBody} `);
        }
        else { 
            bot.reply(message, "Something went wrong... \n*Error:*" + error);
            console.log(`Error retrieving UserInfo on ${userIDToFind} on behalf of ${message.user}`)
         }
    });
}
