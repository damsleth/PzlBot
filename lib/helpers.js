...
helpers.replaceAllInStr = function (s, f, r) {
    return s.replace(new RegExp(f.replace(/([.*+?^=!:${}()|\[\]\/\\])/g, "\\$1"), "g"), r);
};
...
// Returns day names
helpers.getDayName = function (dStr) {
...
// Returns the latest updates from SKAM - here for historical purposes only
helpers.getSKAMUpdates = function () {
...
helpers.mannen = function (bot, message) {
...
helpers.nbim = function (bot, message) {
    fetch("https://www.nbim.no/LiveNavHandler/Current.ashx")
        .then(r => r.json().then(j => {
            bot.reply(message, `Current value of The Government Pension Fund Global\n
*${j.d.liveNavList[0].values[j.d.liveNavList[0].startSecond].Value} NOK*`)
        })).catch(e => console.log(e));
}
// Returns the price of an item from prisjakt.no
helpers.prisjakt = function (query, bot, message) {
...
helpers.svada = function () {
    ...
    svada[0].forEach(function (n, i) {
        s += svada[Math.floor(svada.length * Math.random())][i] + " "
    });
    return s;
}
// Gets the average voting results from pollofpolls.no
helpers.getAveragePoll = function (bot, message) {
    var pollUrl = "http://www.pollofpolls.no/";
...
helpers.fullcontact = function (query, bot, message, isDebug) {
    var isPerson;
    var endpointUrl;
    ...
    function callback(error, response, body) {
        if (!error && response.statusCode == 200) {
            var reply = body;
            if (!isDebug) {
                if (isPerson) reply = helpers.fullcontactPersonFormatter(body);
                else reply = helpers.fullcontactCompanyFormatter(body);
            }
...
helpers.fullcontactPersonFormatter = function (res) {
...
helpers.getCurrentUserInfo = function (bot, message) {
    request.get(`https://slack.com/api/users.info?user=${message.user}&token=${process.env.SLACK_TOKEN}`, function (error, response, body) {
        if (!error) {
            ...
            var formattedBody = helpers.replaceAllInStr(body, ",", ",\n");
            formattedBody = helpers.replaceAllInStr(formattedBody, "{", "{\n");
    ...
    if (userToFind.startsWith("<@")) {
        ...
        request.get(`https://slack.com/api/users.list?token=${process.env.SLACK_TOKEN}`, function (error, response, body) {
            if (!error) {
                ...
                if (userInfo) {
                    var formattedBody = helpers.replaceAllInStr(userInfo, ",", ",\n");
                    formattedBody = helpers.replaceAllInStr(formattedBody, "{", "{\n");
