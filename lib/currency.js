...
currency.getExchangeRateCMC = (bot, message) => {
    var currency = message.match[1];
    fetch(`//api.coinmarketcap.com/v1/ticker/${currency}/`).then(j => j.json().then(coinData => {
        bot.reply(message, `*${currency.toUpperCase()}* - $${price_usd} - ${percent_change_1h}% last 1h`);
    }))
}
